...
| QUAL-001 | Hardening pass for validation and states | Backend validation and persistence hardening landed in PR #8; frontend/UI hardening remains pending once `AUTH-002` starts. |
...
| ID | Title | Outcome |
| --- | --- | --- |
| CAT-001 | Category CRUD vertical slice | Merged in PR #8 with backend-first category CRUD API, persistence, migrations, tests, and docs alignment. |
| EXP-001 | Expense CRUD vertical slice | Merged in PR #8 with backend-first expense CRUD, list/query support, ownership checks, migrations, and test coverage. |
| SUB-001 | Subscription CRUD vertical slice | Merged in PR #8 with backend-first subscription CRUD, list/query support, billing-cycle handling, and test coverage. |
| DASH-001 | Dashboard summary vertical slice | Merged in PR #8 with backend dashboard summary aggregates, recent expenses, upcoming renewals, and tests. |
